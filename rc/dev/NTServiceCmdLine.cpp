...
StringType CCmdLine::GetSafeArgument(const char *pSwitch, int iIdx, const char *pDefault)
{
   ...
   return GetArgument(pSwitch, iIdx, pDefault);
}
...
StringType CCmdLine::GetArgument(const char *pSwitch, int iIdx, StringType pDefault)
{
   if (HasSwitch(pSwitch)) {
      ...
      theIterator = find(pSwitch);
		if (theIterator!=end()) {
			if ((*theIterator).second.m_strings.size() > (UINT)iIdx) {
